        ...
        while (paid_money < price) {
            ...
            if (coin == 1.0 || coin == 0.5) {
                if (coin > remaining_amount) {
                    printf("You cannot insert more than the remaining amount (%.2f AED). Please try again.\n", remaining_amount);
                } else {
                    paid_money += coin;
                    remaining_amount -= coin;
                    printf("Inserted %.2f AED. Remaining: %.2f AED.\n", coin, remaining_amount);
                }
            } else {
...
void refill() {
    ...
    // Set random quantities for each ingredient
    coffee_beans = rand() % 101 + 50; // Random quantity between 50 and 150 grams
    water = rand() % 301 + 200;       // Random quantity between 200 and 500 ml
    milk = rand() % 201 + 100;        // Random quantity between 100 and 300 ml
    chocolate = rand() % 101 + 50;    // Random quantity between 50 and 150 ml

    // Display the new quantities
    printf("Ingredients refilled randomly as follows:\n");
